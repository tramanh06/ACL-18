...
public class ChannelNewsAsiaWrapper extends Wrapper {
  ...
  public Article parse(Document jsoupDocument) {
    ...
    String portal = "Channel News Asia";
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("a.article__author-title").text();  
  }

  private String getTitle(Document jsoupDocument) {
    return jsoupDocument.select("h1.article__title").text();
  }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("www.channelnewsasia.com/");
  }
}
