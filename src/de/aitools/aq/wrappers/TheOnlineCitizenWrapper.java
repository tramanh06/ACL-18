...
public class TheOnlineCitizenWrapper extends Wrapper {
  ...
  public Article parse(Document jsoupDocument) {
    ...
    String portal = "The Online Citizen";
  ...
  private String getTitle(Document jsoupDocument) {
    return jsoupDocument.select("h1.name.post-title.entry-title").text();
  }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("www.theonlinecitizen.com/");
  }

  public static void main(String[] args) throws IOException, JAXBException {
    String url = "https://www.theonlinecitizen.com/2016/02/05/student-said-plainclothes-officers-at-school-wore-t-shirts-with-police-at-its-back/";
    Document doc = Jsoup.connect(url)
                        .userAgent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.152 Safari/537.36")
                        .get();

    // StraitsTimesWrapper st = new StraitsTimesWrapper();
    TheOnlineCitizenWrapper st = new TheOnlineCitizenWrapper();
    Article article = st.parse(doc);

    System.out.println("author = "+ article);

    File f = new File("theonlinecitizen.xml"); 
    article.write(f);
  }
